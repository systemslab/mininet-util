...
plot(d1tshifted, d1$V2, type="o", col=colors[1], lty=1, pch=1, lwd=1, cex=0.5,
	axes=F, ann=T, xlab="Time (s)", ylab="Queue (packets)",
	xlim=c(0, min(maxt, max(d1tshifted))), ylim=c(0, maxq))

for (i in 2:length(argv)) {
	lines(unlist(t[i]), unlist(qlens[i]), type="o", lty=i, pch=i, lw=1, cex=0.5, col=colors[i])
}
